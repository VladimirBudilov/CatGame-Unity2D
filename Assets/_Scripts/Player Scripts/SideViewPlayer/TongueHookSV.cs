...
public class TongueHookSV : MonoBehaviour
{
    [SerializeField] private float hookSpeed = 1000f;
    ...
    private void BringEnemy()
    {
        if (wasEnemyHooked)
        {
            var finalPos = new Vector2(originalPos.x + offSetX, originalPos.y);
    ...
    private void StartHooking()
    {
        ...
        rb.AddForce(-transform.right * hookSpeed);
    }
    ...
    private void ReturnHook()
    {
        if (isHooking)
        {
            ...
            if ((hookDistance > maxHookDistance) || wasEnemyHooked)
            {
                ...
                transform.position = new Vector2(transform.parent.position.x, transform.parent.position.y + offSetY);
            }
        }
    }
